from worldmap.core.models import AUTHENTICATED_USERS, ANONYMOUS_USERS, CUSTOM_GROUP_USERS
from worldmap.maps.models import Map, Layer, MapLayer, Contact, ContactRole, \
     get_csw, LayerCategory, LayerAttribute, MapSnapshot, MapStats, LayerStats, CHARSETS
from worldmap.profile.forms import ContactProfileForm
...
import logging
from worldmap.flexidates import FlexiDateFormField
import taggit
from worldmap.maps.utils import forward_mercator
from worldmap.maps.owslib_csw import CswRecord
from django.utils.html import escape
from django.forms.models import inlineformset_factory
from django.core.cache import cache
from worldmap.maps.forms import LayerCreateForm, GEOMETRY_CHOICES
...
from worldmap.maps.gs_helpers import get_sld_for, get_postgis_bbox
from worldmap.maps.encode import num_encode, num_decode
from django.db import transaction
import autocomplete_light
from worldmap.maps.encode import despam, XssCleaner

logger = logging.getLogger("worldmap.maps.views")
...
class LayerForm(forms.ModelForm):
    from worldmap.maps.models import CONSTRAINT_OPTIONS
    ...
    if use_email:
        ...
        for useremail, level in perm_spec['users']:
            try:
                ...
                user = _create_new_user(useremail, layer.title, reverse('worldmap.maps.views.layer_detail', args=(layer.typename,)), layer.owner_id)
    ...
    if use_email:
        ...
        for useremail, level in perm_spec['users']:
            try:
                ...
                user = _create_new_user(useremail, m.title, reverse('worldmap.maps.views.view', args=[m.id]), m.owner_id)
...
def describemap(request, mapid):
    ...
    if request.method == "POST":
        ...
        if map_form.is_valid():
            ...
            return HttpResponseRedirect(reverse('worldmap.maps.views.map_controller', args=(map_obj.id,)))
    else:
        # Show form
...
def upload_layer(request):
    if request.method == 'GET':
        ...
        from worldmap.maps.forms import WorldMapLayerUploadForm
...
def layer_replace(request, layername):
    ...
    if request.method == 'GET':
        ...
        from worldmap.maps.forms import LayerUploadForm
        from worldmap.maps.utils import save
        from django.utils.html import escape
        import os, shutil
...
def _metadata_search(query, start, limit, sortby, sortorder, **kw):
    ...
    if start > 0:
        ...
        result['prev'] = reverse('worldmap.maps.views.metadata_search') + '?' + params

    next_page = csw.results.get('nextrecord', 0)
    if next_page > 0:
        params = urlencode({'q': query, 'start': next_page - 1, 'limit': limit})
        result['next'] = reverse('worldmap.maps.views.metadata_search') + '?' + params
...
def _maps_search(query, start, limit, sort_field, sort_dir):
    ...
    if start > 0:
        prev = max(start - limit, 0)
        params = urlencode({'q': query, 'start': prev, 'limit': limit})
        result['prev'] = reverse('worldmap.maps.views.maps_search') + '?' + params

    next_page = start + limit + 1
    if next_page < map_query.count():
        params = urlencode({'q': query, 'start': next_page - 1, 'limit': limit})
        result['next'] = reverse('worldmap.maps.views.maps_search') + '?' + params
            ...
            for user, user_level in users:
                ...
                if use_email:
                    try:
                        ...
                        userObject = _create_new_user(user, lyr.title, reverse('worldmap.maps.views.layer_detail', args=(lyr.typename,)), lyr.owner_id)
                ...
                for user, user_level in spec['permissions'].get("users", []):
                    ...
                    if use_email:
                        try:
                            ...
                            userObject = _create_new_user(user, m.title, reverse('worldmap.maps.views.view', args=[m.id]), m.owner_id)
...
def create_pg_layer(request):
    ...
    if request.method == 'POST':
        from worldmap.maps.utils import check_projection, create_django_record, get_valid_layer_name
