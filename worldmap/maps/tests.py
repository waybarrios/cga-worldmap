...
import worldmap.maps.models
import worldmap.maps.views

from worldmap.maps.models import Map, Layer, LayerCategory, LayerAttribute, User
from worldmap.maps.utils import get_valid_user, GeoNodeException
...
_gs_resource = MagicMock()
_gs_resource.native_bbox = [1, 2, 3, 4]
_gs_resource.storeType = 'dataStore'
_gs_resource.projection = 'EPSG:4326'

Layer.objects.geonetwork = MagicMock()
Layer.objects.gs_catalog = MagicMock()

Layer.objects.gs_catalog.get_resource.return_value = _gs_resource

worldmap.maps.models.get_csw = MagicMock()
worldmap.maps.models.get_csw.return_value.records.get.return_value.identification.keywords = []
...
worldmap.maps.models.get_csw.return_value.records.get.return_value.distribution.online = [_csw_resource]
from worldmap.maps.utils import forward_mercator, inverse_mercator

DUMMY_RESULT ={'rows': [], 'total':0, 'query_info': {'start':0, 'limit': 0, 'q':''}}

worldmap.maps.views._metadata_search = Mock()
worldmap.maps.views._metadata_search.return_value = DUMMY_RESULT

worldmap.maps.views.get_csw = Mock()
worldmap.maps.views.get_csw.return_value.getrecordbyid.return_value = None
worldmap.maps.views.get_csw.return_value.records.values.return_value = [None]
worldmap.maps.views._extract_links = Mock()
worldmap.maps.views._extract_links.return_value = {}

class MapTest(TestCase):
    """Tests worldmap.maps app/module
    """
...
community."
    ...
    def test_layer_set_default_permissions(self):
        ...
        self.assertEqual(layer.get_gen_level(worldmap.core.models.ANONYMOUS_USERS), layer.LEVEL_READ)
        self.assertEqual(layer.get_gen_level(worldmap.core.models.AUTHENTICATED_USERS), layer.LEVEL_READ)
    ...
    def test_set_layer_permissions(self):
        ...
        # Set the Permissions
        worldmap.maps.views.set_layer_permissions(layer, self.perm_spec)

        # Test that the Permissions for ANONYMOUS_USERS and AUTHENTICATED_USERS were set correctly
        self.assertEqual(layer.get_gen_level(worldmap.core.models.ANONYMOUS_USERS), layer.LEVEL_NONE)
        self.assertEqual(layer.get_gen_level(worldmap.core.models.AUTHENTICATED_USERS), layer.LEVEL_NONE)
        ...
        for username, level in self.perm_spec['users']:
            user = worldmap.maps.models.User.objects.get(username=username)
            print user.username
    ...
    def test_perms_info(self):
        ...
        info = worldmap.maps.views._perms_info(layer, worldmap.maps.views.LAYER_LEV_NAMES)

        # Test that ANONYMOUS_USERS and AUTHENTICATED_USERS are set properly
        self.assertEqual(info[worldmap.maps.models.ANONYMOUS_USERS], layer.LEVEL_READ)
        self.assertEqual(info[worldmap.maps.models.AUTHENTICATED_USERS], layer.LEVEL_READ)
    ...
    def test_metadata_search(self):
        ...
        with patch.object(worldmap.maps.views,'_metadata_search') as mock_ms:
            result = {
                'rows' : [{
            ...
            call_args = worldmap.maps.views._metadata_search.call_args
    ...
    def test_split_query(self):
        ...
        keywords = worldmap.maps.views._split_query(query)
...
class ViewTest(TestCase):
    ...
    def test_new_map_with_layer(self):
        with patch('worldmap.maps.models.Layer.objects.gs_catalog') as mock_gs:
...
from worldmap.maps.forms import JSONField, LayerUploadForm, WorldMapLayerUploadForm
...
class UtilsTest(TestCase):
    ...
    def test_layer_type(self):
        from worldmap.maps.utils import layer_type
    ...
    def test_get_files(self):
        from worldmap.maps.utils import get_files
        import shutil
        import tempfile
    ...
    def test_get_valid_name(self):
        from worldmap.maps.utils import get_valid_name
    ...
    def test_get_valid_layer_name(self):
        from worldmap.maps.utils import get_valid_layer_name
    ...
    def test_cleanup(self):
        from worldmap.maps.utils import cleanup
        ...
        with patch('worldmap.maps.models.Layer.objects.gs_catalog') as mock_catalog:
    ...
    def test_check_geonode_is_up(self):
        ...
        from worldmap.maps.utils import check_geonode_is_up
        from mock import patch
        ...
        with nested(
            patch('worldmap.maps.models.Layer.objects.gs_catalog'),
            patch('worldmap.maps.models.Layer.objects.geonetwork')
        ) as (mock_gs, mock_gn):
    ...
    def test_save(self):
        ...
        import tempfile
        from contextlib import nested
        from worldmap.maps.utils import save
        ...
        try:
            ...
            with nested(
                patch.object(worldmap.maps.models, '_wms', new=MockWMS()),
                patch('worldmap.maps.models.Layer.objects.gs_catalog'),
                patch('worldmap.maps.models.Layer.objects.geonetwork')
